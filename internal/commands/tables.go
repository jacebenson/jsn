...
func printMarkdownSchema(cmd *cobra.Command, table *sdk.Table, parentChain []chainItem, childTables []sdk.Table, instanceURL string) error {
	...
	if len(childTables) > 0 {
		fmt.Fprintln(cmd.OutOrStdout(), fmt.Sprintf("#### Extends To (%d)", len(childTables)))
		fmt.Fprintln(cmd.OutOrStdout())
		for _, child := range childTables {
			childScope := child.Scope
			if childScope == "" {
...
// newTablesChoicesCmd creates the tables choices command.
func newTablesChoicesCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "choices <table> <column>",
		Short: "Show choice values for a column",
		Long:  "Display choice values (dropdown options) for a specific column from the sys_choice table.",
		Args:  cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			return runTablesChoices(cmd, args[0], args[1])
		},
	}
}

// runTablesChoices executes the tables choices command.
func runTablesChoices(cmd *cobra.Command, tableName, columnName string) error {
	appCtx := appctx.FromContext(cmd.Context())
	if appCtx == nil {
		return fmt.Errorf("app not initialized")
	}
	...
	// Get choice values
	choices, err := sdkClient.GetColumnChoices(cmd.Context(), tableName, columnName)
	if err != nil {
	...
	if format == output.FormatStyled || (format == output.FormatAuto && isTerminal) {
		return printStyledChoices(cmd, tableName, columnName, choices, instanceURL)
	}

	if format == output.FormatMarkdown {
		return printMarkdownChoices(cmd, tableName, columnName, choices, instanceURL)
	}
	...
	for _, choice := range choices {
		item := map[string]string{
			"value":    choice.Value,
			"label":    choice.Label,
			"sequence": fmt.Sprintf("%d", choice.Sequence),
		}
		if choice.Dependent != "" {
	...
	breadcrumbs := []output.Breadcrumb{
		{
			Action:      "schema",
			Cmd:         fmt.Sprintf("jsn tables schema %s", tableName),
			Description: fmt.Sprintf("View table schema (%s)", tableName),
		},
		{
			Action:      "columns",
			Cmd:         fmt.Sprintf("jsn tables columns %s", tableName),
			Description: "View all columns",
		},
	}

	return outputWriter.OK(data,
		output.WithSummary(fmt.Sprintf("%d choices for %s.%s", len(choices), tableName, columnName)),
		output.WithBreadcrumbs(breadcrumbs...),
	)
}

// printStyledChoices outputs styled choice values.
func printStyledChoices(cmd *cobra.Command, tableName, columnName string, choices []sdk.ChoiceValue, instanceURL string) error {
	headerStyle := lipgloss.NewStyle().Bold(true).Foreground(output.BrandColor)
	labelStyle := lipgloss.NewStyle().Foreground(lipgloss.Color("#888888"))
	valueStyle := lipgloss.NewStyle()
	...
	// Choices list
	for _, choice := range choices {
		choiceDisplay := fmt.Sprintf("%-20s", choice.Value)
		labelDisplay := choice.Label
		if choice.Sequence > 0 {
			labelDisplay = fmt.Sprintf("%s (#%d)", labelDisplay, choice.Sequence)
		}
		if choice.Dependent != "" {
			labelDisplay = fmt.Sprintf("%s [depends: %s]", labelDisplay, choice.Dependent)
		}

		fmt.Fprintf(cmd.OutOrStdout(), "  %s  %s\n",
			valueStyle.Render(choiceDisplay),
			labelStyle.Render(labelDisplay),
		)
	}
...
// printMarkdownChoices outputs markdown choice values.
func printMarkdownChoices(cmd *cobra.Command, tableName, columnName string, choices []sdk.ChoiceValue, instanceURL string) error {
	title := fmt.Sprintf("**%s.%s Choices**", tableName, columnName)
	fmt.Fprintf(cmd.OutOrStdout(), "%s\n\n", title)

	fmt.Fprintln(cmd.OutOrStdout(), "| Value | Label | Seq |")
	fmt.Fprintln(cmd.OutOrStdout(), "|-------|-------|-----|")

	for _, choice := range choices {
		seq := ""
		if choice.Sequence > 0 {
			seq = fmt.Sprintf("%d", choice.Sequence)
		}
		fmt.Fprintf(cmd.OutOrStdout(), "| %s | %s | %s |\n", choice.Value, choice.Label, seq)
	}
