...
func (m *mockTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	m.capturedPaths = append(m.capturedPaths, req.URL.Path)
	m.capturedQuery = req.URL.RawQuery
	m.capturedQueries = append(m.capturedQueries, req.URL.RawQuery)
	...
	if len(m.responseStatuses) > 0 && m.requestCount <= len(m.responseStatuses) {
		status = m.responseStatuses[m.requestCount-1]
	}

	body := m.responseBody
	// Check if we have a specific body for this request
	if len(m.responseBodies) > 0 && m.requestCount <= len(m.responseBodies) {
		body = m.responseBodies[m.requestCount-1]
	}
...
// capturedAnyPath checks if any of the captured paths contain the given substring
func (m *mockTransport) capturedAnyPath(substring string) bool {
	for _, path := range m.capturedPaths {
